...
func TestFactory_CreateConnection(t *testing.T) {
	t.Run("成功创建连接", func(t *testing.T) {
		factory := NewFactory()
		instance := types.NewDatabaseInstance("test-ch", types.DatabaseTypeClickHouse, "localhost", 9000)
...
// --- buildDSN 间接测试（通过 Connection 内部方法）---
...
func TestConnection_BuildDSN(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "带数据库名",
			instance: makeInstance("host", 9000, "user", "pass", "mydb"),
			wantDSN:  "clickhouse://user:pass@host:9000/mydb",
		},
		{
			name:     "不带数据库名",
			instance: makeInstance("host", 9000, "user", "pass", ""),
			wantDSN:  "clickhouse://user:pass@host:9000",
		},
		{
			name:     "空用户名密码",
			instance: makeInstance("127.0.0.1", 8123, "", "", "default"),
			wantDSN:  "clickhouse://:@127.0.0.1:8123/default",
		},
	}
...
func makeInstance(host string, port int, username, password, database string) *types.DatabaseInstance {
	inst := types.NewDatabaseInstance("test", types.DatabaseTypeClickHouse, host, port)
