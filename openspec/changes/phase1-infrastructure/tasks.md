...
- [ ] 5.2 实现 root 命令 internal/cli/root.go（全局标志、配置加载）
- [ ] 5.3 实现 version 命令 internal/cli/version.go（显示版本信息）
- [ ] 5.4 初始化日志系统（支持 verbose 标志）
- [ ] 5.5 测试 CLI 基础命令

## 6. CLI - Instances 命令
- [ ] 6.1 实现 instances list 命令 internal/cli/instances.go
- [ ] 6.2 支持表格格式输出（默认）
- [ ] 6.3 支持 JSON 格式输出（--format json）
- [ ] 6.4 支持 CSV 格式输出（--format csv）
- [ ] 6.5 实现 instances get 命令（获取单个实例详情）
- [ ] 6.6 测试 instances 命令

## 7. CLI - Query 命令
- [ ] 7.1 实现 query 命令 internal/cli/query.go
- [ ] 7.2 集成 MySQL 连接执行查询
- [ ] 7.3 实现表格格式输出（默认）
- [ ] 7.4 实现 JSON 格式输出
- [ ] 7.5 实现 CSV 格式输出
...
## 8. REST API - 服务器设置
- [ ] 8.1 创建 API 服务器 internal/api/rest/server.go（Gin 初始化、路由设置）
- [ ] 8.2 实现优雅关闭逻辑（SIGTERM/SIGINT 处理）
- [ ] 8.3 添加 CORS 中间件
- [ ] 8.4 添加日志中间件（记录请求信息）
- [ ] 8.5 添加错误恢复中间件
- [ ] 8.6 配置 Gin 模式（debug/release）
- [ ] 8.7 测试服务器启动和关闭

## 9. REST API - 健康检查端点
- [ ] 9.1 实现 GET /health 端点 internal/api/rest/handlers.go
- [ ] 9.2 实现健康检查响应格式
- [ ] 9.3 实现 check-instances 参数支持
- [ ] 9.4 测试健康检查端点

## 10. REST API - Instances 端点
- [ ] 10.1 实现 GET /api/v1/instances 端点
- [ ] 10.2 实现实例列表响应格式
- [ ] 10.3 实现密码脱敏逻辑
- [ ] 10.4 测试 instances 端点

## 11. REST API - Query 端点
- [ ] 11.1 实现 POST /api/v1/query 端点
- [ ] 11.2 定义请求和响应结构（QueryRequest、QueryResponse）
- [ ] 11.3 集成 MySQL 连接执行查询
- [ ] 11.4 实现 context 超时控制
- [ ] 11.5 实现错误响应格式
- [ ] 11.6 测试 query 端点

## 12. 集成和端到端测试
- [ ] 12.1 在 main.go 中组装所有组件（配置、发现、连接、CLI、API）
- [ ] 12.2 测试完整流程：配置加载 → 实例发现 → 连接建立 → 查询执行
- [ ] 12.3 测试 CLI 完整工作流（mystisql instances list、mystisql query）
- [ ] 12.4 测试 API 完整工作流（health、instances、query 端点）
- [ ] 12.5 测试错误处理和边界情况

## 13. 代码质量和文档
- [ ] 13.1 运行 go fmt ./... 格式化代码
- [ ] 13.2 运行 go vet ./... 检查代码
- [ ] 13.3 运行 golangci-lint run 并修复问题
- [ ] 13.4 为所有导出函数和类型添加文档注释
- [ ] 13.5 检查代码是否遵循 AGENTS.md 规范
- [ ] 13.6 确保 Context 传递正确
- [ ] 13.7 确保错误处理包含上下文信息
- [ ] 13.8 确保不记录敏感信息（密码、token）

## 14. 最终验证
- [ ] 14.1 更新 README.md 添加 Phase 1 使用示例
- [ ] 14.2 创建详细的配置文件注释示例
- [ ] 14.3 手动测试：编译成功（go build）
- [ ] 14.4 手动测试：CLI 命令正常工作
- [ ] 14.5 手动测试：API 端点正常工作
- [ ] 14.6 手动测试：能够连接真实 MySQL 数据库
- [ ] 14.7 手动测试：查询执行正确
- [ ] 14.8 验证所有 Phase 1 目标已完成
