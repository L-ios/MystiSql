...
- [ ] 10.1 实现批量 SQL 执行器（支持 INSERT、UPDATE、DELETE）
- [ ] 10.2 实现混合批处理（支持不同类型的 SQL）
- [ ] 10.3 实现批量操作大小限制（默认 1000）
- [ ] 10.4 实现 REST API：POST /api/v1/batch（批量执行）
- [ ] 10.5 实现批量操作错误处理（部分成功返回详细结果）
- [ ] 10.6 优化批量操作性能（使用数据库原生批处理）
- [ ] 10.7 编写批量操作单元测试

## 11. CLI 认证集成

- [ ] 11.1 实现 CLI Token 配置管理（配置文件、环境变量、命令行参数）
- [ ] 11.2 实现 CLI Token 优先级逻辑（命令行 > 环境变量 > 配置文件）
- [ ] 11.3 为 query 命令添加 --token 参数
- [ ] 11.4 为 instances 命令添加 --token 参数
- [ ] 11.5 实现 auth 子命令：token（生成 Token）
- [ ] 11.6 实现 auth 子命令：revoke（撤销 Token）
- [ ] 11.7 实现 auth 子命令：info（查看 Token 信息）
- [ ] 11.8 实现 CLI 认证失败处理和错误提示
- [ ] 11.9 编写 CLI 认证集成测试
...
- [ ] 13.1 编写端到端测试：Token 认证流程
- [ ] 13.2 编写端到端测试：审计日志记录
- [ ] 13.3 编写端到端测试：SQL 验证拦截
- [ ] 13.4 编写端到端测试：WebSocket 查询
- [ ] 13.5 编写端到端测试：JDBC 事务
- [ ] 13.6 编写端到端测试：JDBC 批量操作
- [ ] 13.7 编写端到端测试：PostgreSQL 连接
- [ ] 13.8 性能测试：认证中间件延迟 < 1ms
- [ ] 13.9 性能测试：审计日志异步写入不阻塞请求
- [ ] 13.10 安全测试：Token 泄露防护、SQL 注入防护

## 14. 集成和发布

- [ ] 14.1 运行所有单元测试（go test ./...）
- [ ] 14.2 运行代码检查（golangci-lint run）
- [ ] 14.3 运行代码格式化（go fmt ./...）
- [ ] 14.4 更新版本号（v0.3.0）
- [ ] 14.5 编译发布版本（Linux、macOS、Windows）
- [ ] 14.6 更新 CHANGELOG.md
- [ ] 14.7 创建 Git tag（v0.3.0-phase3-security-layer）
