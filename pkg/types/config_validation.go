...
func (c *Config) validateInstances() error {
	...
	for _, instance := range c.Instances {
		...
		if instance.Role == InstanceRoleSlave {
			slaveConfigs = append(slaveConfigs, instance)
		} else if instance.Role == InstanceRoleMaster || instance.Role == "" {
...
func (c *Config) GetTopologyGroups() map[string]*TopologyGroup {
	...
	for _, instance := range c.Instances {
		if instance.Role == InstanceRoleMaster || instance.Role == "" {
	...
	for _, instance := range c.Instances {
		if instance.Role == InstanceRoleSlave && instance.Master != "" {
...
func (c *Config) GetSlavesByMaster(masterName string) []InstanceConfig {
	slaves := make([]InstanceConfig, 0)
	for _, instance := range c.Instances {
		if instance.Role == InstanceRoleSlave && instance.Master == masterName {
			slaves = append(slaves, instance)
		}
	}
	return slaves
